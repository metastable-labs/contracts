...
contract LaunchboxFactoryUnit is LaunchboxFactoryBase {
    function test_DeployWithInvalidToken() public {
        ...
        LaunchboxFactory testLaunchpad = new LaunchboxFactory(address(0), address(curveImpl), mockUniswapRouter, marketCapThreshold, platfromFeePercentage);
    }
    ...
    function test_DeployWithInvalidLaunchboxExchange() public {
        ...
        LaunchboxFactory testLaunchpad = new LaunchboxFactory(address(erc20Impl), address(0), mockUniswapRouter, marketCapThreshold, platfromFeePercentage);
    }

    function test_DeployWithInvalidRouter() public {
        vm.expectRevert(LaunchboxFactory.EmptyUniswapRouter.selector);
        LaunchboxFactory testLaunchpad = new LaunchboxFactory(address(erc20Impl), address(curveImpl), address(0), marketCapThreshold, platfromFeePercentage);
    }

    function test_DeployWithGreaterThan100Fee() public {
        vm.expectRevert(LaunchboxFactory.FeeGreaterThanHundred.selector);
        LaunchboxFactory testLaunchpad = new LaunchboxFactory(address(erc20Impl), address(curveImpl), address(0), marketCapThreshold, 101 * 1e18);
    }
